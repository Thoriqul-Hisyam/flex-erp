...
import { Plus, CheckCircle, XCircle, Edit3, Trash2, Lock, Download, Upload, FileSpreadsheet, Info, Check, AlertCircle } from "lucide-react";
...
import { createRecordAction, updateRecordAction, deleteRecordAction, fetchRecordsAction } from "@/app/actions/crud-actions";
...
export function MasterDataPage<T extends MasterDataItem>({
  ...
  const handleExportExcel = () => {
    if (data.length === 0) {
      showToast({ type: "error", title: "Export Failed", message: "No data to export." });
      return;
    }
    ...
    showToast({ type: "success", title: "Exported", message: `${data.length} ${entityName} records exported to CSV.` });
    logAuditEvent({
  ...
  const handleDownloadTemplate = () => {
    ...
    const csvRows = [
      fieldNames.join(","),
      fieldNames.map((fn) => `"${sampleRow1[fn].replace(/"/g, '""')}"`).join(","),
      fieldNames.map((fn) => `"${sampleRow2[fn].replace(/"/g, '""')}"`).join(","),
    ];
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      ...
      if (lines.length < 2) {
        showToast({ type: "error", title: "Invalid File", message: "CSV file must contain headers and at least 1 data row." });
        return;
      }
  ...
  const handleOpenEdit = (item: T) => {
    ...
    createFields.forEach((f) => {
      if (f.type === "select" && editForm[f.name] !== undefined && editForm[f.name] !== null) {
  ...
  const handleSubmitForm = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      if (editingItem) {
        const res = await updateRecordAction(entityName, editingItem.id, formData);
        if (res.success) {
          showToast({ type: "success", title: "Berhasil Diperbarui", message: `Data ${entityName} "${formData.name || formData.code}" telah berhasil diperbarui.` });
          setIsModalOpen(false);
          const refresh = await fetchRecordsAction(entityName);
          if (refresh.success) setData((refresh.data || []) as T[]);
        } else {
          showToast({ type: "error", title: "Gagal Memperbarui", message: res.error || "Gagal memperbarui data." });
        }
      } else {
        const res = await createRecordAction(entityName, formData);
        if (res.success) {
          showToast({ type: "success", title: "Berhasil Ditambahkan", message: `Data ${entityName} "${formData.name || formData.code}" telah berhasil dibuat.` });
          setIsModalOpen(false);
          const refresh = await fetchRecordsAction(entityName);
          if (refresh.success) setData((refresh.data || []) as T[]);
        } else {
          showToast({ type: "error", title: "Gagal Menambahkan", message: res.error || "Gagal membuat data baru." });
        }
      }
    });
  };
  ...
  const handleDelete = (id: string) => {
    ...
    startTransition(async () => {
      ...
      if (res.success) {
        showToast({ type: "success", title: "Berhasil Dihapus", message: `Data ${entityName} telah berhasil dihapus.` });
        setConfirmDeleteId(null);
        const refresh = await fetchRecordsAction(entityName);
        if (refresh.success) setData((refresh.data || []) as T[]);
      } else {
        showToast({ type: "error", title: "Gagal Menghapus", message: res.error || "Gagal menghapus data." });
      }
    });
  };

  // Guard: if user's role has no READ permission for this page, display Unauthorized card
      ...
      accessor: (item) => (
        <div className="flex items-center justify-end gap-1" onClick={(e) => e.stopPropagation()}>
          ...
          {!permission.canUpdate && !permission.canDelete && (
            <span title="Akses hanya baca" className="p-1.5 text-slate-300 dark:text-slate-700">
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Add button */}
          {permission.canCreate && (
            <Button onClick={handleOpenCreate} size="sm" className="h-10 px-5 gap-2 bg-[#0088ff] hover:bg-[#0077ee] text-white font-semibold text-xs rounded-full shadow-md shadow-blue-500/20 cursor-pointer">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-[#12161f] p-4 rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-[#e6e9f0] dark:border-slate-800 flex items-center justify-between">
          <div>
            <div className="text-[10px] font-bold text-[#8a94a6] uppercase tracking-wider">Total Data</div>
            <div className="text-2xl font-bold font-mono-num text-[#0f172a] dark:text-white mt-0.5">{totalCount}</div>
          </div>
        ...
        <div className="bg-white dark:bg-[#12161f] p-4 rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-[#e6e9f0] dark:border-slate-800 flex items-center justify-between">
          <div>
            <div className="text-[10px] font-bold text-[#8a94a6] uppercase tracking-wider">Data Aktif</div>
            <div className="text-2xl font-bold font-mono-num text-[#10b981] mt-0.5">{activeCount}</div>
          </div>
        ...
        <div className="bg-white dark:bg-[#12161f] p-4 rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-[#e6e9f0] dark:border-slate-800 flex items-center justify-between">
          <div>
            <div className="text-[10px] font-bold text-[#8a94a6] uppercase tracking-wider">Non-Aktif / Penangguhan</div>
            <div className="text-2xl font-bold font-mono-num text-[#ef4444] mt-0.5">{inactiveCount}</div>
          </div>
      ...
      <Modal
        ...
        title={editingItem ? `Edit ${entityName}: ${editingItem.code || editingItem.name}` : `Create New ${entityName}`}
        description={editingItem ? "Perbarui data record yang sudah ada." : "Isi formulir di bawah untuk membuat record baru."}
      >
        <form onSubmit={handleSubmitForm} className="space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-4">
            {createFields.map((field) => {
              const isDisabled = editingItem ? field.disabled || field.disabledOnEdit : field.disabled;
              const isFullWidth = field.type === "textarea";

              const fieldWrapper = (content: React.ReactNode) => (
                <div key={field.name} className={cn("space-y-1.5", isFullWidth && "sm:col-span-2")}>
            ...
            <div className="space-y-1.5">
              <label className="text-[11px] font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wide">Record Status</label>
              <select
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-700/50">
            <Button type="button" variant="outline" onClick={() => setIsModalOpen(false)} className="rounded-lg text-xs h-9 px-4 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer">
              Batal
            </Button>
            <Button type="submit" disabled={isPending} className="bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-semibold h-9 px-5 shadow-sm shadow-blue-500/20 cursor-pointer disabled:opacity-50">
              {isPending ? "Menyimpan..." : editingItem ? "Simpan Perubahan" : "Tambah Data"}
            </Button>
          </div>
        </form>
      </Modal>
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 rounded-2xl bg-amber-50/60 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50 flex items-start gap-3">
            ...
            <div className="space-y-1">
              <h4 className="text-xs font-bold text-amber-900 dark:text-amber-200">Panduan Impor Data CSV</h4>
              <p className="text-[11px] text-amber-700 dark:text-amber-300">
                Unduh template CSV terlebih dahulu untuk melihat struktur kolom yang sesuai (seperti <strong>code</strong>, <strong>name</strong>, dsb). Isikan data Anda lalu unggah file CSV di bawah ini.
              </p>
              <Button
                type="button"
          ...
          <div className="space-y-2">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Pilih File CSV Data Anda</label>
            <input
              ref={fileInputRef}
              type="file"
              accept=".csv"
            ...
            >
              ...
              <p className="text-xs font-semibold text-slate-700 dark:text-slate-300">
                {selectedFile ? selectedFile.name : "Klik di sini untuk memilih file .csv"}
              </p>
              <p className="text-[10px] text-slate-400 mt-1">Format file didukung: CSV (UTF-8)</p>
            </div>
          </div>
                  ...
                  <thead className="bg-slate-100 dark:bg-slate-800 font-bold border-b border-slate-200 dark:border-slate-700">
                    <tr>
                      {Object.keys(previewRows[0]).map((h) => (
                        <th key={h} className="px-3 py-1.5 uppercase text-[10px]">{h}</th>
                      ))}
                    </tr>
                  </thead>
                    ...
                    {previewRows.map((r, idx) => (
                      <tr key={idx}>
                        {Object.values(r).map((v, valIdx) => (
                          <td key={valIdx} className="px-3 py-1.5">{v}</td>
                        ))}
                      </tr>
                    ))}
                  </tbody>
      ...
      {confirmDeleteId && (
        <Modal isOpen={!!confirmDeleteId} onClose={() => setConfirmDeleteId(null)} title={`Konfirmasi Hapus Data ${entityName}`}>
          <div className="space-y-4">
            <p className="text-xs text-slate-600 dark:text-slate-400">
              Apakah Anda yakin ingin menghapus data {entityName} ini? Tindakan ini tidak dapat dibatalkan.
            </p>
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
              <Button variant="outline" onClick={() => setConfirmDeleteId(null)} className="rounded-xl text-xs cursor-pointer">
                Batal
              </Button>
              <Button
